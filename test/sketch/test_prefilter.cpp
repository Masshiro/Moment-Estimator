...
void task_function(const string& data_path, const string& grouptruth_path, uint32_t label) {
    Aton_vHLL *sketch = new Aton_vHLL(STAGE_NUM, STAGE_ROW, STAGE_COL/2, rd());
    CardinalityMap tmpcard;
    spdlog::debug("{} - {}", data_path, grouptruth_path);
    ifstream infs(data_path);
    ifstream true_infs(grouptruth_path);
    int true_card = 0;
    double est_card;
    double r_err;
    string src_ip;
...
TEST(OVHT4bit, prefilter) {
    ...
    while (ifls.good()) {
        ...
        string true_data_path =
            fmt::format("{}{}grouptruth", DATA_DIR, readline);
