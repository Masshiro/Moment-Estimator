...
//  Experiment Settings
//===============================================================
const string TEST_STAMP = "50_files-exp1-";
...
void run_one_file_data_once(string data_file, int file_index, int round_count, string true_file) {
    for (int i = 0; i < prog_rate.size(); ++i) {
...
//                    rd(), prog_rate[i]);
            ...
            hierarchy::MomentEstimatorPro<On_vHLL>* sketchPro = new hierarchy::MomentEstimatorPro<On_vHLL>(
                    TOP_K, level_cnt[i], STAGE_NUM, sketch_settings[j][0], sketch_settings[j][1],
                    rd(), true_file, prog_rate[i]
            );

            std::ifstream infs(data_file);
            std::string src_ip;
            std::string dst_ip;
            if(!infs){
                std::cout << "Open file \' "<< data_file <<"\' failed!"<<std::endl;
                infs.close();
            }
...
//    return 1;
    ...
    while (input_list.good()){
        ...
        std::string data_file = fmt::format("{}{}uniq", DATA_DIR, one_file);
        std::string true_file = fmt::format("{}{}groundtruth", DATA_DIR, one_file);
//        std::cout << "Processing file: " << filename <<std::endl;

        for (int i = 0; i < REPEAT_TIMES; ++i) {
            auto taskfun = std::bind(run_one_file_data_once, data_file, file_index, i, true_file);
