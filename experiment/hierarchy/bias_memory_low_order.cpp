//
// Created by Masshiro on 2023/3/20.
//
#include <iostream>
#include <fstream>
#include <vector>
...
const uint8_t MOMENT_COUNT = 8;

const vector<int> moment_degree = {0, 1, 3, 5, 7, 10, 12, 15};
...
// Exporting Settings
//===============================================================
const std::string OUTPUT_DIR = "../../result/hierarchy/lower_order_no_halving/";
const vector<string> moment_name = {"-0-", "-1-", "-3-", "-5-", "-7-", "-10-", "-12-", "-15-"};
        ...
        prog_rate.size(), vector<vector<vector<double>>> (
                FILE_NUM, vector<vector<double>> (
                        sketch_settings.size(), vector<double> (
                                1+MOMENT_COUNT, 0.0))) );
vector<vector<vector<vector<double>>>> AllResults2(
        prog_rate.size(), vector<vector<vector<double>>> (
                FILE_NUM, vector<vector<double>> (
                        sketch_settings.size(), vector<double> (
                                1+MOMENT_COUNT, 0.0))) );
...
//            hierarchy::LUSSketch<Ton_vHLL>* sketchPro = new hierarchy::LUSSketch<Ton_vHLL>(
//                    TOP_K, level_cnt[i],
...
//                    rd(), prog_rate[i]
//                    );
            hierarchy::MomentEstimator<Ton_vHLL>* sketchPro = new hierarchy::MomentEstimator<Ton_vHLL>(
                    TOP_K, level_cnt[i], STAGE_NUM, sketch_settings[j][0], sketch_settings[j][1],
                    rd(), prog_rate[i]
            );
            ...
            for (int k = 0; k < MOMENT_COUNT; ++k) {
                ...
                AllResults2[i][file_index][j][k] += pow(tmp, 2);
            }
            AllResults[i][file_index][j].back() += sketchPro->memory_usage_int_bits() / 8388608.0;
            AllResults2[i][file_index][j].back() += sketchPro->memory_usage_int_bits() / 8388608.0;
        ...
        for (int j = 0; j < AllResults[i].size(); ++j) {
            for (int k = 0; k < AllResults[i][j].size(); ++k) {
                for (int l = 0; l < AllResults[i][j][k].size(); ++l) {
                    ...
                    AllResults2[i][j][k][l] /= REPEAT_TIMES;
                }
            }
        }
    }
...
void export_results() {
    std::ofstream* outfs1 = new std::ofstream [prog_rate.size() * moment_degree.size()];
    std::ofstream* outfs2 = new std::ofstream [prog_rate.size() * moment_degree.size()];
        ...
        for (int j = 0; j < AllResults[i].size(); ++j) {
            ...
            for (int k = 0; k < AllResults[i][j].size(); ++k) {
                ...
                for (int l = 0; l < AllResults[i][j][k].size()-1; ++l) {
                    ...
                    outfs2[i*MOMENT_COUNT+l] << AllResults2[i][j][k][l] << ' ' << AllResults2[i][j][k].back() << std::endl;
                }
            }
        }
    }
