//
// Created by Masshiro on 2022/9/26.
//
...
void task_function(const string data_path, const string groundtruth_path, uint32_t seed) {
    ...
    while (true_infs.good()) {
        ...
        // spdlog::debug("query : {} :> {} : {}", src_ip, est_card, true_card); if (est_card == )
        r_err = get_r_error((double)true_card, est_card);
        tmpcard.add(true_card, r_err);
    }
    spdlog::info("task:{} end", seed);
    infs.close();
    true_infs.close();
    delete sketch;
